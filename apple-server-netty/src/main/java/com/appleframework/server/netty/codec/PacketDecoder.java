...
package com.appleframework.server.netty.codec;

import static com.appleframework.server.netty.protocol.Packet.decodePacket;

import java.util.List;

import com.appleframework.server.netty.protocol.JsonPacket;
...
public final class PacketDecoder extends ByteToMessageDecoder {
    ...
    private Packet decodeFrame(int dataLen, byte[] request) {
		if (dataLen > maxPacketSize) {
			throw new TooLongFrameException("packet body length over limit:" + dataLen);
		}
		return decodePacket(new Packet(), request);
	}

	/*private Packet decodeFrame(ByteBuf in) {
		int readableBytes = in.readableBytes();
		if (readableBytes < Packet.DATA_LEN) {
			return null;
		}
		if (readableBytes > maxPacketSize) {
			throw new TooLongFrameException("packet body length over limit:" + readableBytes);
		}
		return decodePacket(new Packet(in.readByte()), in, readableBytes);
	}*/
    ...
    /*public static Packet decodeFrame(DatagramPacket frame) {
        ...
        if (readableBytes < (bodyLength + Packet.DATA_LEN)) {
            return null;
        }
